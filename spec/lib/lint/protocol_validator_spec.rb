...
module Lint
  describe ProtocolValidator do
    ...
    context 'when it encounters a protocol without properties' do
      it 'displays a no protocol defined error' do
    ...
    context 'when it validates files' do
      ...
      it 'displays an error when multiple entry points are specified' do
    ...
    context 'when it encounters an invalid protocol' do
      it 'displays an exception error ' do
        filename = lint_spec_filename('protocol_section_errors', 'invalid_protocol.yml')
        expect { capture(:stdout) { Lint.validate(filename) } }.to raise_error
