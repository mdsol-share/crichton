...
class Crichton::Middleware::ServiceResourcesCatalog < Crichton::Middleware::MiddlewareBase
  ...
  # @example
  #   config.middleware.use Crichton::Middleware::ServiceResourceCatalog, {'expiry' => 20}
  #
  ...
  def call(env)
    ...
    req.path == '/' ? process_response(env) : @app.call(env)
  end
  ...
  def process_response(env)
    ...
    if content_type_sym = response_media_type_sym(media_type)
      respond(media_type, content_type_sym)
    else
