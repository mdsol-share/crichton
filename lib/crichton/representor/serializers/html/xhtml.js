$(document).ready(function () {
    var CrichtonAjax = {
        build_selector_callback: function(name, value, link) {
        ...
        update_browser_url: function(context) {
            var self_href = $('a[rel="self"]', context).attr('href');
            history.pushState({}, '', self_href);
        },

        replace_callback: function(selector) {
            var result = function(data) {
                var content = $(data).filter(selector);
                $(selector).replaceWith(content);
                CrichtonAjax.update_browser_url(content);
            }
            return result;
        },

        beforesend_callback: function(xhr, settings) {
        ...
        call: function(method, url, send, dataType) {
            var result = function(success){
                $.ajax({
                    ...
                    data: send,
                    dataType: dataType,
                    beforeSend: CrichtonAjax.beforesend_callback,
                    success: success
                });
            };
            return result;
    ...
    $('.save').click(function() {
        var name = $('.element-name').val(),
            ...
            div = create_div_element({ class: 'input-group ' + subtype + '-' + name });
        if (name == '' || value == '') {
            $('.alert').show();
            return false;
        };
    ...
    $('form').submit(function() {
        var form = $(this);
        add_fields_to_the_form(form);
        var func = CrichtonAjax.call(form.attr('method'), form.attr('action'), form.serialize(), 'html');
        func(CrichtonAjax.replace_callback('.main-content'));
        return false;
    });

    $('a').click(function() {
        var link = $(this);
        var url = add_parameters_to_url(link.attr('href'));
        link.attr('href', url);
    ...
    $('a[target]').each(function() {
        ...
        func(CrichtonAjax.build_selector_callback(name, value, link));
    });
});
...
function build_control_group(element_group) {
    var div_control_group = create_div_element({ class: 'control-group' }),
        label = create_label_element({ for: element_group }, capitalizeFirstLetter(element_group)),
        div_controls = create_div_element({ class: 'controls' }),
        input = create_input_element({ class: 'span5 element-' + element_group, type: 'text', name: 'value' }),
        span = create_span_element({ class: 'help-block hide' });
